...
MongoClient.connect(url3, function(err) {
	if(err) {
		...
		dbo.collection("collectionName").findOne({}, function(err1, res){
		//dbo.collection("collectionName").findOne({field:data}, function(err1, res){			//select * where 
		//dbo.collection("collectionName").findOne({},{table:{field:data}}, function(err1, res){  // select column2 from table
			if(err1){
