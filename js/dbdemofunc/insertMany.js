...
MongoClient.connect(url3, function(err) {
	if(err) {
		...
		var myobj = [{/*object data*/}, {/*object2 data*/}];
		dbo.createCollection("collectionName").insertMany(myobj, function(err1, res){
			if(err1){
				console.log('Error inserting documents!');
				throw err1;
			}
			else{
				console.log('New documents inserted succesfully!');
			}
			db.close();
		});
	}
